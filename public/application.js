...
var findEvent = function findEvent(id) {
  return $.ajax({
    url: config.apiOrigin + '/events/' + id,
    method: 'GET',
    headers: {
...
var onSignIn = function onSignIn(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.signIn(data).then(ui.signInSuccess).then(ui.checkEvents).catch(ui.signInFailure);
...
var onShowEvents = function onShowEvents(event) {
  event.preventDefault();
  api.showEvents().then(ui.showSuccess).catch(ui.showFailure);
};

var onCreateEvent = function onCreateEvent(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.createEvent(data).then(ui.createSuccess).catch(ui.createFailure);
...
var onUpdateEvent = function onUpdateEvent(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.updateEvent(data).then(ui.updateSuccess).catch(ui.updateFailure);
};

var onFindEvent = function onFindEvent(event) {
  store.title = $('#find-title').val();
  event.preventDefault();
  api.showEvents().then(ui.otherFindSuccess).catch(ui.showFailure);
};
...
var populateUpdateFields = function populateUpdateFields(id) {
  api.findEvent(id).then(findSuccess).catch(findFailure);
};
...
var checkEvents = function checkEvents() {
  api.showEvents().then(checkSuccess).catch(showFailure);
};
...
var showSuccess = function showSuccess(data) {
  ...
  $('.remove').on('click', function (e) {
    e.preventDefault();
    api.deleteEvent($(e.target).parent().data('id')).then(deleteSuccess(e)).then(checkEvents).catch(deleteFailure);
  });
...
var updateSuccess = function updateSuccess() {
  ...
  api.showEvents().then(showSuccess).catch(showFailure);
};
...
var otherFindSuccess = function otherFindSuccess(data) {
  console.log(data.events);
  var foundData = data.events.filter(function (event) {
    return event.title === store.title;
  });
  if (foundData === []) {
...
module.exports = {
  ...
  otherFindSuccess: otherFindSuccess,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n   <!-- Developers, why are we using data-id and not just id? -->\n   <div class=\"events\" data-id=\""
    ...
    + "</p>\n     <p>Dresscode: "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.dresscode : stack1), depth0))
    + " Attendees: "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.attendees : stack1), depth0))
    + "</p>\n     <button class=\"bton remove\" type=\"button\" name=\"button\">Remove</button>\n     <button class=\"bton update\" type=\"button\" name=\"button\">Update</button>\n   </div>\n\n";
